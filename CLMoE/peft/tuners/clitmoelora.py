...
class CLMoEMOELoraConfig(LoraConfig):
    ...
    warmup_tokens: int = field(default=0, metadata={"help": "Warmup tokens for MoE gating"})
...
class TSMoERouter(nn.Module):
    def __init__(self, config, in_features):
        ...
        self.threshold = getattr(config, "warmup_tokens", 0)
...
class TransientSIManager:
    ...
    # accumulate_step 现在不需要手动调用了，留空或者删除
...
class CLMoEMOELoraLinear(nn.Linear, CLMoEMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.warmup_tokens = kwargs.pop('warmup_tokens', 0)
...
class CLMoEMOELoraModel(LoraModel):
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            ...
            "warmup_tokens": getattr(lora_config, "warmup_tokens", 0)
        }
...
# ==========================================
# 5. Visualization Helper (Debug Version)
# ==========================================
# def save_layer_masks_as_image(expert_masks, layer_name, step, save_dir="./vis_masks"):
#     """
#     可视化并保存掩码为图像文件。
#     改进版：将同层同参数的所有 Expert Mask 画在同一张大图上 (Grid Layout)，并使用【统一色标】。
#     """
#     try:
...
#         # 2. 获取所有可能存在的参数名 (取并集)
#         all_param_names = set()
#         for mask_dict in expert_masks:
#             if mask_dict:
#                 all_param_names.update(mask_dict.keys())
        
#         if not all_param_names:
#             return

#         # 3. 针对每个参数名，画一张包含所有 Expert 的大图
...
#                     if mask_dict[param_name].dim() == 2:
#                         is_2d = True
#                     break
#             if not is_2d:
#                 continue
                
#             # --- [关键步骤] 3.1.5 预先计算全局统一的最大值 (Global Max) ---
#             # 这一步是为了让 CKA 高的 Expert 看起来亮，CKA 低的看起来暗
#             all_data_for_this_param = []
#             for i in range(num_experts):
#                 if expert_masks[i] and param_name in expert_masks[i]:
#                     tensor = expert_masks[i][param_name]
#                     data = tensor.detach().cpu().float().numpy()
#                     all_data_for_this_param.append(data)
            
#             if not all_data_for_this_param:
#                 continue # 没有任何专家有这个参数的数据
            
#             # 计算所有专家里最大的那个值作为统一的天花板
#             # 使用 max() 可能会被极个别噪点拉太高，导致整体全黑
#             # 推荐使用 99% 分位数 (np.percentile) 来做 Robust Max，或者直接 max() 看需求
#             # 这里为了保险起见，先用绝对 max，如果你发现太黑看不清，可以改成 np.percentile(..., 99)
#             global_max = max([d.max() for d in all_data_for_this_param])
#             if global_max == 0: global_max = 1.0

#             # --- 3.2 动态计算子图布局 (Rows x Cols) ---
...
#             fig.suptitle(f"Layer: {layer_name} | Param: {param_name} | Step: {step}", fontsize=14)
...
#                 if mask_dict and param_name in mask_dict:
#                     tensor = mask_dict[param_name]
#                     data = tensor.detach().cpu().float().numpy()
                    
#                     sparsity = (data == 0).mean()
                    
#                     # [关键修改] 使用全局统一的 vmax=global_max
#                     # 这样 CKA 低的 Expert (数值小) 就会自动变暗
#                     sns.heatmap(data, 
#                                 vmin=0, 
#                                 vmax=global_max, 
#                                 cmap="viridis", 
...
#                     has_valid_plot = True
#                 else:
#                     ax.text(0.5, 0.5, "No Mask", ha='center', va='center', color='gray')
#                     ax.set_title(f"Expert {i} (Empty)", fontsize=10)
#                     ax.axis('off') # 空图直接关掉坐标轴
...
#                 filename = f"{save_dir}/{safe_layer}_{safe_param}.png"
                
#                 plt.tight_layout()
#                 plt.subplots_adjust(top=0.9)
#                 plt.savefig(filename, dpi=150) # 提高一点清晰度
#                 print(f" [Vis] Saved grid mask (Unified Scale): {filename}")
            
#             plt.close(fig)

#     except Exception as e:
#         import traceback
#         print(f"[Vis Error] Failed to plot masks: {e}")
#         traceback.print_exc()