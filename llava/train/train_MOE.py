...
class CPMoEDataset(Dataset):
    """Dataset for supervised fine-tuning with precise SuperNI formatting."""
    ...
    def __init__(self, data_path: str,
        ...
        # 2. 精准格式转换 (SuperNI -> LLaVA/LLaMA-2 格式)
        list_data_dict = []
        
        # 判断是否为你提供的 SuperNI 字典格式
        if isinstance(loaded_data, dict) and 'Instances' in loaded_data:
            # 提取任务指令 (Definition 通常是一个列表，我们取第一项)
            ...
            for instance in loaded_data['Instances']:
                ...
                # 【关键修复】你数据中的 output 是一个列表，必须提取里面的字符串
            ...
            raise ValueError("Unrecognized data format. Expected a dict with 'Instances' or a list.")

        # 3. 处理连续学习的 memory 数据 (如果有)
        if getattr(data_args, 'memory_data_path', None) is not None:
            with open(data_args.memory_data_path, "r", encoding="utf-8") as f:
                list_memory_data_dict = json.load(f)
            # 假设 memory_data 已经是转换好的格式
        ...
        # 4. Warmup 估算与复制逻辑 (保持不变，此时 'conversations' 键已绝对安全)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Data is wrapped into a list."
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    # [补丁] 加载 Mask (为了防止遗忘)
    # =========================================================
    mask_file = os.path.join(previous_task_model_path, "clmoe_masks.pt")
    
    if os.path.exists(mask_file):
        print(f"loading masks: {mask_file}")
        try:
            clmoe_state = torch.load(mask_file, map_location="cpu")
            
            # 找到 base_model (兼容 PeftModel 包装)
            base_model_obj = model.base_model if hasattr(model, "base_model") else model
            
            activated_count = 0
            for name, module in base_model_obj.named_modules():
                # 尝试匹配 mask
                matched_key = None
                if name in clmoe_state:
                    matched_key = name
                else:
                    # 模糊匹配 (防止前缀不一致)
                    for key in clmoe_state.keys():
                        if name.endswith(key) or key.endswith(name):
                            matched_key = key
                            break
                
                # 如果找到了 mask 且模块支持操作
                if matched_key and hasattr(module, "load_masks"):
                    module.load_masks(clmoe_state[matched_key])
                    # 关键一步：Commit! 把 Pending Mask 转为正式约束
                    if hasattr(module, "commit_masks"):
                        module.commit_masks()
                        activated_count += 1
            
            print(f"loaded {activated_count}  Mask regularization modules from previous task.")
            
        except Exception as e:
            print(f"mask loading : {e}")
    else:
        print("No mask file found, skipping mask loading.")
    
    print("Updating Anchors for Regularization...")
    
    # 再次获取 base_model (以防变量作用域问题)
    base_model_obj = model.base_model if hasattr(model, "base_model") else model
    
    anchor_count = 0
    for name, module in base_model_obj.named_modules():
        # 寻找实现了 update_all_anchors 方法的层 (也就是 CLMoELoraLinear)
        if hasattr(module, "update_all_anchors"):
            # 将当前加载进来的 Task 1 权重，复制一份存入 anchor_params
            module.update_all_anchors()
            anchor_count += 1
            
    if anchor_count > 0:
        print(f"Successfully created anchors for {anchor_count} layers. Regularization is now active.")
    else:
        print("Warning: No layers found to update anchors! (Check if adapter names match)")
    print("Model loading from previous task is complete.")
    print('Model is fully loaded (Weights + Masks + Anchors)...')
...
def train():
    ...
    if training_args.lora_enable:
        ...
        if targets is None:
             # targets = find_all_linear_names(model) # 全量
             targets = ["gate_proj", "up_proj", "down_proj"] # 或者默认只MLP
 ...
 # =========================================================
    ...
    # 2. 提交 Mask (Commit)
    # =========================================================
    # 必须先提交，把 Pending Buffer 转为正式约束
    # 简单解包，兼容 DDP/DeepSpeed
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank in [-1, 0]:
            ...
            # B. 保存 CLMoE Masks
            clmoe_state = {}
            # 再次解包 PeftModel 以遍历底层层级
            base_model = real_model.base_model if hasattr(real_model, "base_model") else real_model
            
            for name, module in base_model.named_modules():
                if hasattr(module, "export_masks"):
                    mask_data = module.export_masks()
                    if mask_data: 
                        clmoe_state[name] = mask_data
            
            if clmoe_state:
                torch.save(clmoe_state, os.path.join(training_args.output_dir, 'clmoe_masks.pt'))
                print(f"✅ Saved CLMoE masks for {len(clmoe_state)} layers.")
            else:
                print("⚠️ Warning: No masks collected.")
