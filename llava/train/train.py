...
def train():
    ...
    if training_args.lora_enable:
        from CLMoE.peft.tuners.TSmoe import CLMoEMOELoraConfig, CLMoEMOELoraModel
        ...
        #     task_type="CAUSAL_LM",
        # )
        #这里改为了设配TS  怎么快怎么来  后续再优化设配不同lora模型
        kwargs = { 
        ...
        lora_config = CLMoEMOELoraConfig(
            ...
            # --- CL-MoE 特有参数 ---
            expert_num=4,             # 专家数量
            task_embedding_dim=64,    # TE 维度
            warmup_tokens=100000,        # Warmup 步数 (建议先设小一点测试)
            **kwargs
        )
        ...
        model = CLMoEMOELoraModel(model, {"default": lora_config}, "default")
